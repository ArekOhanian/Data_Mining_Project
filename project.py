...
#4/30/26 Jason Luu classification imports
...
class ShopWiseClassification:
    ...
    def prepare_classification_data(self):
        ...
        #create target variable (satisfaction level from Review Rating)
        if 'Review Rating' in self.data.columns:
            #convert continuous rating to categorical
            self.data['Satisfaction_Level'] = pd.cut(
                self.data['Review Rating'],
                bins=[-np.inf, 2.5, 3.5, np.inf],
                labels=['Low', 'Medium', 'High']
            )
            y = self.data['Satisfaction_Level']
        ...
        encoded_features = [col for col in self.data.columns if '_encoded' in col]
        feature_cols.extend(encoded_features[:5])

        X = self.data[feature_cols].copy()
        print(f"Features for classification: {feature_cols}")
    ...
    def train_and_evaluate(self, test_size=0.2, random_state=42):
        ...
        X, y = self.prepare_classification_data()
...
#4/30/26
    #classification
    classification = ShopWiseClassification(processed_data)
    classification_results = classification.train_and_evaluate()
    ...
    # generating association rules can change the confidence
